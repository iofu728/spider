...
# @Last Modified time: 2021-06-21 17:54:32
...
def get_gpa(cookie: str):
    ...
    score_list = [
        (int(ii["xf"]), ii["cj"]) for ii in score_list if ii["cj"][0] in need_cj
    ]
    ...
    echo(
        1,
        f"{name}, Congratulations u get {TC} credits and {gpa:.3f} gpa in this university.",
    )
    for ii in need_cj:
...
if __name__ == "__main__":
    ...
    parser = create_argparser("pku student helper")
    parser.add_argument(
        "--mode", type=int, default=1, metavar="mode", help="0->bruchclass,1->get_gpa"
    )
    parser.add_argument(
        "--cookie", type=str, default="", metavar="cookie", help="portal cookie"
    )
