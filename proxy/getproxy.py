...
# @Last Modified time: 2019-09-13 12:18:53
...
class GetFreeProxy:
    ...
    def update_proxy(self, update_list: list, types: int):
        ...
        if results:
            echo('2|info', 'Update', typemap[types],str(len(update_list)),' items Success!')
    ...
    def thread_judge(self, batch_size: int = 500):
        ...
        for block in range(num // batch_size + 1):
            proxy_th = [proxy_exec.submit(self.judge_url, jj, ii, 0) for ii, jj in enumerate(text[block * batch_size:batch_size * (block + 1)])]
    ...
    def get_other_proxies(self, url: str):
        ''' get other proxies '''
        pages = re.findall(r'<h3[\s\S]*?<a.*?(http.*?\.html).*?</a>', self.request_text(url))
        if not len(pages):
